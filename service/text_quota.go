...
func calculateTextToolCallSurcharge(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, summary *textQuotaSummary) decimal.Decimal {
	...
	if relayInfo.ResponsesUsageInfo != nil {
		for name, tool := range relayInfo.ResponsesUsageInfo.BuiltInTools {
			...
			if name == dto.BuildInToolImageGeneration && relayInfo.PriceData.ImageSizePricing {
				// 图片尺寸定价已包含本次生图成本，不再叠加通用工具调用附加费。
				continue
			}
...
func calculateTextQuotaSummary(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage) textQuotaSummary {
	...
	if summary.TotalTokens == 0 && relayInfo.PriceData.ImageSizePricing {
		// 图片尺寸价格在请求转发前已经锁定；兼容上游即使缺失 usage，成功请求仍需结算已选定的固定价格。
