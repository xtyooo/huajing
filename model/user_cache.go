...
func (user *UserBase) WriteContext(c *gin.Context) {
	...
	common.SetContextKey(c, constant.ContextKeyM, user.M)
}
...
func populateUserCache(user User) error {
	...
	return writeUserCacheWithCustomFields(user, true)
}
...
func updateUserCache(user User) error {
	...
	return writeUserCacheWithCustomFields(user, false)
}

// writeUserCacheWithCustomFields 通过官方认证版本栅栏原子写入用户缓存及二开的倍率字段。
func writeUserCacheWithCustomFields(user User, includeQuota bool) error {
	return writeUserCache(user.ToBaseUser(), includeQuota)
}
...
func getUserMCache(userId int) (float64, error) {
	cache, err := GetUserCache(userId)
	if err != nil {
		return 0, err
	}
...
// updateUserMCache 通过认证版本栅栏更新用户倍率，避免旧快照覆盖新权限状态。
func updateUserMCache(userId int, m float64) error {
	return updateUserCacheField(userId, "M", fmt.Sprintf("%g", m))
}
