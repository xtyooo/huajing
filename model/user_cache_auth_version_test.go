...
// TestUserCachePreservesCustomMultiplier 验证二开用户倍率会随认证版本缓存原子写入并可单独刷新。
func TestUserCachePreservesCustomMultiplier(t *testing.T) {
	truncateTables(t)
	useUserCacheMiniRedis(t)

	user := User{
		Username:    "custom-multiplier-cache",
		Password:    "password",
	...
	assert.InDelta(t, 1.25, cached.M, 0.000001)

	require.NoError(t, updateUserMCache(user.Id, 2.75))
	cached, err = cacheGetUserBase(user.Id)
	require.NoError(t, err)
	assert.InDelta(t, 2.75, cached.M, 0.000001)
}
