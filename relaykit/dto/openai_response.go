...
// IsSuccessfulImageGenerationCall 判断当前输出项是否为可计费的成功图片生成调用。
...
// GetQuality 返回第一个成功图片生成结果的质量参数，供兼容计费逻辑使用。
...
// GetSize 返回第一个成功图片生成结果的尺寸参数，供兼容计费逻辑使用。
