...
func (r *GeneralOpenAIRequest) GetTokenCountMeta() *types.TokenCountMeta {
	...
	if len(r.ExtraBody) > 0 {
		...
		if err := kitutil.Unmarshal(r.ExtraBody, &extraBody); err == nil {
	...
	if len(r.Modalities) > 0 {
		...
		if err := kitutil.Unmarshal(r.Modalities, &modalities); err == nil {
...
func (r *OpenAIResponsesRequest) GetTokenCountMeta() *types.TokenCountMeta {
	...
	for _, tool := range r.GetToolsMap() {
		if kitutil.Interface2String(tool["type"]) != "image_generation" {
			continue
		}
		meta.ImageGeneration = true
		meta.ImageSize = kitutil.Interface2String(tool["size"])
