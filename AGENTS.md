...
This repository is maintained by one person and intentionally uses a simple direct-to-dev workflow.

### Permanent branch

- `dev` is the only permanent development and release branch. The Gitee default branch and local `origin/HEAD` must point to `dev`.
- Normal features, fixes, documentation, channel integrations, tests, and deployment tooling are developed directly on `dev`. Do not create long-lived `feat/*`, `fix/*`, `main`, `integrate/*`, or upgrade branches for routine work.
- Before starting work, run `git fetch origin`, switch to `dev`, and update with `git pull --ff-only origin dev`. Never start from an older release branch merely because it is already checked out locally.
- A task is not complete until its intended commits are present on local `dev` and pushed to `origin/dev`. Deployment from an unpushed temporary branch is prohibited.
...
- The temporary branch must be merged back into `dev`, fully verified, pushed, and deleted locally and remotely in the same task. Do not leave temporary branches or worktrees behind for later cleanup.
- Production releases must be built from a clean commit on `dev`. The release commit/tag must be contained in `origin/dev` before deployment begins.

### Worktree and dirty-state safety

- Use one canonical `dev` worktree for day-to-day development. Do not create a new worktree for each channel or fix.
...
1. Verify the current branch is `dev` and its base includes the latest production commit.
2. Run the tests, vet/lint/type checks, and build required by the changed area.
3. Commit the scoped files directly to `dev` using the repository's commit style.
