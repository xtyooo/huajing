...
// MaskSensitiveInfo 复用 RelayKit 的敏感信息脱敏实现，保证主模块与协议转换模块的输出规则一致。
func MaskSensitiveInfo(str string) string {
	return kitutil.MaskSensitiveInfo(str)
}

// MaskRequiredQuotaAmount 复用 RelayKit 的公开错误脱敏规则，避免 Task 路径与协议转换路径产生差异。
func MaskRequiredQuotaAmount(str string) string {
	return kitutil.MaskRequiredQuotaAmount(str)
}
